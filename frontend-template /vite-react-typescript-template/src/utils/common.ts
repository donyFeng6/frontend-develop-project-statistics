...
export function publicOptions(params: any, isSign?: boolean) {
  ...
  const app_key =
    baseParam.app_key || Utils.getUrlParam('appKey') || params.app_key
  const data: PublicParams = {
    app_key,
    client_os_type: params.client_os_type || 19,
    device_id: params.device_id || Utils.getUUID(),
...
/** 参数签名 */
export function setSignature(params: Record<string, any>, appSecret: string) {
  const signature = CryptoJS.enc.Base64.stringify(
    CryptoJS.enc.Utf8.parse(Utils.jsonSort(params, '&'))
  )
  const sha1ResultBytes = CryptoJS.MD5(CryptoJS.HmacSHA1(signature, appSecret))
